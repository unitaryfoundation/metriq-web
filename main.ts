...
let metriqGymSuiteMetadata: MetriqGymSuiteMetadata | null = null;
...
let platformSortKey: PlatformSortKey = 'score';
let platformSortDir: PlatformSortDirection = 'desc';
...
let datasetGeneratedDateIso: string | null = null;
...
function activateView(which: 'results'|'platforms'|'benchmarks'|'news', skipHashUpdate = false) {
  ...
  if (!skipHashUpdate) {
    if (isPlatforms) updatePlatformsListHash();
    else updateHash({ view: which });
  }
}
...
function applyPlatformListRoute(route: Record<string, string>) {
...
function setMetriqGymSuiteMetadata(suiteDefinition: unknown) {
  const metadata = resolveMetriqGymSuiteMetadata(suiteDefinition);
  if (metadata) metriqGymSuiteMetadata = metadata;
  return metadata;
}

function invalidateMetriqGymSuiteMetadata() {
  metriqGymSuiteMetadata = null;
  document.querySelectorAll<HTMLElement>('[data-suite-version-context]').forEach((element) => {
    element.remove();
  });
}

function renderSuiteVersionControlHtml() {
  if (!metriqGymSuiteMetadata) return '';
  return `
...
async function rerenderPlatformsRoute(h: Record<string, string> = parseHash()) {
  ...
  if (helpTopic === 'metriq-score') {
    ...
    void loadAppConfig().then(() => {
      if (parseHash().help === 'metriq-score') renderMetriqScoreHelp();
...
function coverageExclusionNotes(coverage: PlatformCoverage): string[] {
  const notes: string[] = [];
  if (coverage.unsupported > 0) notes.push(`${coverage.unsupported} not supported`);
  if (coverage.notApplicable > 0) notes.push(`${coverage.notApplicable} not applicable`);
  if (coverage.errored > 0) notes.push(`${coverage.errored} with a reported error`);
  return notes;
}
...
function withAdjustedMetriqScore(detail: any): any {
  if (recordAggMode !== 'all-time' || !benchmarkRunsByGroup) return detail;
...
const REPORTED_OUTCOME_LABELS: Record<RecordOutcome, string> = {
  unsupported: 'Not supported',
  error: 'Error reported',
  not_applicable: 'N/A',
};

const REPORTED_OUTCOME_DESCRIPTIONS: Record<RecordOutcome, string> = {
...
function reportedOutcomeChipClass(outcome: RecordOutcome) {
  return outcome === 'error' ? 'component-status--error' : 'component-status--na';
}

type PlatformComponentContext = { provider: string; device: string; group: string };
...
function findReportedOutcomeRun(context: PlatformComponentContext | null, timestamp: string | null) {
  if (!context || !timestamp || !benchmarkRunsByGroup) return null;
...
function reportedOutcomeTooltip(
  availability: PlatformScoreComponentAvailability,
  context: PlatformComponentContext | null,
) {
  ...
  if (availability.reportedOutcomeReason) lines.push(`Reason: ${availability.reportedOutcomeReason}`);
  ...
  if (errorMessage) lines.push(`Error: ${truncateText(String(errorMessage))}`);
...
function renderPlatformComponentStatusHtml(
  ...
  context: PlatformComponentContext | null = null,
  options: { inlineDate?: boolean } = {},
) {
...
function renderCompareComponentWeights(name: string, leftComponents: Record<string, any>, rightComponents: Record<string, any>) {
  const weights = [leftComponents[name]?.weight, rightComponents[name]?.weight].map((value) =>
  ...
  if (weights[0] === weights[1]) return format(weights[0]);
...
function renderCompareComponentRow(label: string, weightHtml: string, aHtml: string, bHtml: string, differenceHtml: string, aHref = '', bHref = '') {
  return renderCompareComponentRowHtml(escapeHtml(label), weightHtml, aHtml, bHtml, differenceHtml, aHref, bHref);
}

function renderCompareComponentRowHtml(labelHtml: string, weightHtml: string, aHtml: string, bHtml: string, differenceHtml: string, aHref = '', bHref = '', rowClass = '') {
...
function renderCompareComponentDifferenceHtml(leftValue: number | null, rightValue: number | null) {
...
function buildCompareComponentOutcomeHash(
  provider: string,
  device: string,
  componentName: string,
  component: any,
...
function renderPlatformComparePage(left: any, right: any) {
  ...
  const overlapRow = renderCompareComponentRowHtml(
    `${renderCompareOverlapScoreLabelHtml()}<div class="compare-subvalue">${escapeHtml(overlapCountLabel)}</div>`,
    '–',
    renderCompareMaybeBetterNumber(leftOverlapScore, rightOverlapScore, 2),
    renderCompareMaybeBetterNumber(rightOverlapScore, leftOverlapScore, 2),
    renderCompareComponentDifferenceHtml(leftOverlapScore, rightOverlapScore),
    '',
    '',
    'compare-component-row--overlap',
  );
...
function renderPlatformComponentNotesHtml(derivedUnsupportedCount: number, reportedOutcomeCount: number) {
  const notes: string[] = [];
...
function renderPlatformDetailPage(detail: any, suiteDefinition: unknown = null) {
  const container = document.getElementById('platforms-container');
  if (!container) return;
  ...
  if (metriqScore && typeof metriqScore === 'object') {
    ...
    const rows = components.map(([name, c]) => {
      ...
      if (isUnsupported && !reportedOutcome) derivedUnsupportedCount += 1;
      if (reportedOutcome) reportedOutcomeCount += 1;
      ...
      let statusHtml = renderPlatformComponentStatusHtml(
        availability,
        detailDeviceQubits,
        { provider: String(provider), device: String(device), group: benchmark },
      );
      const rowClasses = ['platform-component-row'];
      if (isUnsupported || availability.status === 'not_applicable') rowClasses.push('platform-component-row--unsupported');
  ...
  try {
    ...
    if (!metriqGymSuiteMetadata) {
      void suiteDefinitionPromise.then((definition) => {
        ...
        if (definition && isPlatformsList) renderPlatformsTable();
      });
    }
  } catch (err) {
  ...
  if (!appConfigPromise) {
    appConfigPromise = (async () => {
      try {
        ...
        setMetriqGymSuiteMetadata((appConfigCache as any).metriqGymSuite);
...
function renderRunOutcomeChipHtml(run: any) {
...
function renderRunOutcomeHtml(run: any, outcome: RecordOutcome) {
  ...
  if (detail.reason) rows.push(`<tr><td>Reason</td><td>${escapeHtml(detail.reason)}</td></tr>`);
  if (detail.source) rows.push(`<tr><td>Source</td><td>${escapeHtml(detail.source)}</td></tr>`);
...
function applyTableFilters(values: any[]) {
  ...
  return values.filter(v => {
    ...
    if (q) {
      const blob = `${v.timestamp||''} ${v.provider||''} ${v.device||''} ${v.benchmark||''} ${v.num_qubits ?? ''} ${v.outcome ? REPORTED_OUTCOME_LABELS[v.outcome as RecordOutcome] : ''}`.toLowerCase();
