...
export type BenchmarkRunIndex = Map<string, Array<{ run: any; sig: string }>>;
...
function rawDirection(run: any, metric: string): string | null {
 ...
 */
export function adjustMetriqScoreForRecords(detail: any, runsIndex: BenchmarkRunIndex): any {
  const score = detail?.metriq_score;
  const components = score?.components;
  if (!components || typeof components !== 'object' || Array.isArray(components)
    || !runsIndex?.size || typeof runsIndex.get !== 'function') return detail;
  ...
  if (calculateMetriqScore(components) === null) return detail;

  const provider = String(detail?.provider || '');
  const device = String(detail?.device || '');
  const rawGroups = new Set(Object.values(components).filter((component: any) => (
    component?.group != null && component?.aggregation !== 'harmonic'
    && finiteNumber(component?.raw) !== null
  )).map((component: any) => component.group));
  const adjustedComponents: Record<string, any> = { ...components };
  let changed = false;

  for (const [name, component] of Object.entries(components) as Array<[string, any]>) {
    ...
    if (!anchor || typeof anchor.sig !== 'string') continue;
    ...
    for (const entry of entries) {
      if (entry?.sig !== anchor.sig || !entry?.run?.timestamp) continue;
      const run = entry.run;
      ...
      if (requiresRaw && (raw === null || baseline === null || direction === null)) continue;
      if (normalized === null && !(selfBaseline && requiresRaw && raw !== null)) continue;
