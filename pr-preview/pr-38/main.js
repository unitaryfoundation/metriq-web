...
const datasetGeneratedAt = document.getElementById('dataset-generated-at');
const datasetGeneratedDate = document.getElementById('dataset-generated-date');
...
const chartTitleEl = panelGraph?.querySelector('.panel__title') || null;
const downloadChartBtn = document.getElementById('btn-download-chart');
const downloadChartMenu = document.getElementById('chart-download-menu');
const downloadChartRoot = document.getElementById('chart-download');
...
let platformsIndexByKeyCache = null;
let platformScoresCache = null;
let platformQubitsCache = null;
let platformCoverageCache = null;
let platformDetailsCache = null;
let platformSortKey = 'score';
let platformSortDir = 'desc';
let platformProviderFilter = '';
let showRetiredDevices = true;
...
function renderDatasetGeneratedDate(value) {
    if (!datasetGeneratedAt || !datasetGeneratedDate)
        return;
...
function setPlatformsIndexCache(platforms) {
...
function getPlatformLifecycle(provider, device, source) {
...
function formatLifecycleEffectiveAt(value) {
    if (!value)
        return '';
...
function renderLifecycleBadgeHtml(lifecycle) {
...
function renderDeviceBadgesHtml(provider, device, source) {
    const badges = [];
...
function renderDeviceLabelHtml(provider, device, source) {
    return `${escapeHtml(device || '')}${renderDeviceBadgesHtml(provider, device, source)}`;
}
function renderLifecycleNoteHtml(provider, device, source) {
    const lifecycle = getPlatformLifecycle(provider, device, source);
...
function setChartDownloadEnabled(enabled) {
    if (downloadChartBtn)
...
function buildChartDownloadName(ext) {
...
function closeChartDownloadMenu() {
    if (!downloadChartMenu || !downloadChartBtn)
        return;
...
async function downloadChartImage(ext) {
    if (!chartView)
        return;
    try {
        if (ext === 'svg') {
...
downloadChartBtn?.addEventListener('click', (event) => {
    if (!downloadChartBtn || downloadChartBtn.disabled)
        return;
...
downloadChartMenu?.addEventListener('click', (event) => {
    ...
    const btn = target && target.closest ? target.closest('button[data-format]') : null;
...
document.addEventListener('click', (event) => {
    if (!downloadChartRoot || !downloadChartMenu || downloadChartMenu.hidden)
        return;
    const target = event.target;
    if (!target)
        return;
    if (!downloadChartRoot.contains(target))
        closeChartDownloadMenu();
});
...
function renderMultiList(listId, options, selected, kind, searchTerm) {
    ...
    options.forEach(opt => {
        if (term && !opt.toLowerCase().includes(term))
            return;
...
function toggleMultiSelection(key, options, value) {
    ...
    if (set.has(value))
        ...
        set.add(value);
    filterState[key] = Array.from(set);
}
function renderMultiLists() {
    const allRuns = Array.isArray(rawBenchmarks) ? rawBenchmarks : [];
    const providers = uniqueValues(allRuns, 'provider');
    const benchmarks = uniqueValues(allRuns, 'benchmark');
    ...
    if (!multiBootstrapped) {
        if (!filterState.provider || filterState.provider.length === 0)
            filterState.provider = providers.slice();
        if (!filterState.benchmark || filterState.benchmark.length === 0)
            filterState.benchmark = benchmarks.slice();
        multiBootstrapped = true;
    }
    const benchSearchEl = document.getElementById('filter-search-benchmark');
    const provSearchEl = document.getElementById('filter-search-provider');
    ...
    if (benchCount)
        benchCount.textContent = `${filterState.benchmark.length} of ${benchmarks.length}`;
    if (provCount)
        provCount.textContent = `${filterState.provider.length} of ${providers.length}`;
    // Wire actions — "Select all" / "Deselect all" only affect visible (filtered) items
    const pClear = document.getElementById('provider-clear');
    const pAll = document.getElementById('provider-all');
    const bClear = document.getElementById('benchmark-clear');
    const bAll = document.getElementById('benchmark-all');
    ...
    if (pClear) {
        pClear.classList.toggle('is-current', noVisibleProvSelected);
        pClear.onclick = (event) => { event.preventDefault(); event.stopPropagation(); filterState.provider = filterState.provider.filter(p => !visibleProvSet.has(p)); renderMultiLists(); drawChart(); };
    }
    if (pAll) {
        pAll.classList.toggle('is-current', allVisibleProvSelected);
        pAll.onclick = (event) => { event.preventDefault(); event.stopPropagation(); filterState.provider = visibleProviders.slice(); renderMultiLists(); drawChart(); };
    }
    if (bClear) {
        bClear.classList.toggle('is-current', noVisibleBenchSelected);
        bClear.onclick = (event) => { event.preventDefault(); event.stopPropagation(); filterState.benchmark = filterState.benchmark.filter(b => !visibleBenchSet.has(b)); renderMultiLists(); drawChart(); };
    }
    if (bAll) {
        bAll.classList.toggle('is-current', allVisibleBenchSelected);
        bAll.onclick = (event) => { event.preventDefault(); event.stopPropagation(); filterState.benchmark = visibleBenchmarks.slice(); renderMultiLists(); drawChart(); };
    }
}
...
function applyRecordModeFromRoute(route) {
...
function setRecordAggMode(mode) {
    if (recordAggMode === mode)
        return;
    recordAggMode = mode;
    syncRecordModeToggle();
    ...
    if (viewResults && !viewResults.hidden) {
        ...
        if (panelGraph?.classList.contains('is-active'))
            void drawChart();
        if (panelTable?.classList.contains('is-active'))
            void drawTable();
    }
...
document.addEventListener('click', (event) => {
    const target = event.target;
    const btn = target?.closest?.('[data-record-mode]');
    if (!btn)
        return;
    const mode = btn.getAttribute('data-record-mode');
    if (mode === 'all-time' || mode === 'latest')
        setRecordAggMode(mode);
});
syncRecordModeToggle();
...
function buildFallbackUpdateId(item, index) {
...
function focusUpdateFromHash(section, track) {
    const targetId = String(parseHash().update || '').trim();
    if (!targetId)
        return;
    const cards = Array.from(track.querySelectorAll('[data-update-id]'));
...
(async () => {
    ...
    const wireDownload = (selector, url, fallbackName, preferFallbackName = false) => {
    ...
    const MetriqTourCtor = window.MetriqTour;
    ...
    try {
        ...
        if ('view' in next) {
            ...
            if (next.view !== 'results') {
                ...
                if (!('results_provider' in next))
                    delete merged.results_provider;
                if (!('results_device' in next))
                    delete merged.results_device;
                if (!('results_benchmark' in next))
                    delete merged.results_benchmark;
                if (!('results_timestamp' in next))
                    delete merged.results_timestamp;
                if (!('results_tab' in next))
                    delete merged.results_tab;
            }
        }
        const hasScopedRoute = Boolean(merged.provider
            || merged.device
            || merged.help
        ...
        Object.entries(merged).forEach(([k, v]) => { if (v != null && v !== '')
            p.set(k, v); });
        const nh = '#' + p.toString();
        if (location.hash !== nh)
...
function appendRecordModeParam(params) {
...
function buildPlatformsHelpHash(topic) {
    const params = new URLSearchParams({ view: 'platforms', help: topic });
    appendRecordModeParam(params);
    return '#' + params.toString();
}
function navigateToPlatform(provider, device) {
...
function buildResultsHash(provider, device, benchmark, timestamp = '', tab = 'table') {
    ...
    if (timestamp)
        params.set('results_timestamp', timestamp);
    if (tab === 'table')
        params.set('results_anchor', 'table');
    appendRecordModeParam(params);
    return '#' + params.toString();
}
...
function scrollHelpPanelIntoView(container) {
    requestAnimationFrame(() => {
        const panel = container.querySelector('.detail-page');
...
async function rerenderPlatformsRoute(h = parseHash()) {
...
function extractPlatformNumQubits(detail) {
...
function deviceMetadataNumQubits(detail) {
...
function extractPlatformCoverage(detail) {
    ...
    if (!values.length)
        return null;
    const hasFinite = (value) => value !== null && value !== undefined && Number.isFinite(Number(value));
...
async function loadPlatformScores() {
    ...
    if (!platformScoresCache)
        platformScoresCache = new Map();
    if (!platformQubitsCache)
        platformQubitsCache = new Map();
    if (!platformCoverageCache)
        platformCoverageCache = new Map();
    ...
    await Promise.all(platforms.map(async (p) => {
        ...
        try {
            ...
            if (!resp.ok)
                return;
            const json = await resp.json();
            if (!platformDetailsCache)
                platformDetailsCache = new Map();
...
function buildBenchmarkRunIndex(runs) {
    const index = new Map();
    runs.forEach((run) => {
...
function formatPlatformComponentRawValue(value) {
    if (value === null || value === undefined)
        return '–';
    const num = Number(value);
...
function buildPlatformDetailUrl(provider, device, indexUrl) {
    const base = getPlatformsBaseUrl(indexUrl) || 'https://unitaryfoundation.github.io/metriq-data/platforms';
    return `${base}/${encodeURIComponent(provider)}/${encodeURIComponent(device)}.json`;
}
async function loadPlatformDetail(provider, device) {
    if (!platformDetailsCache)
        platformDetailsCache = new Map();
    const key = getDeviceKey(provider, device);
...
function buildPlatformDetailHash(provider, device) {
    const params = new URLSearchParams({ view: 'platforms', provider, device });
    appendRecordModeParam(params);
    return '#' + params.toString();
}
function buildCompareHash(providerA, deviceA, providerB, deviceB) {
    const params = new URLSearchParams({
        view: 'platforms',
...
function buildCompareHashFromRoute(route) {
    ...
    if (!providerA || !deviceA || !providerB || !deviceB)
        return '';
    return buildCompareHash(providerA, deviceA, providerB, deviceB);
}
function buildOverlapScoreHelpHashFromRoute(route) {
    const params = new URLSearchParams({
        view: 'platforms',
...
function renderPlatformOptionLabel(platform) {
    const provider = String(platform?.provider || '');
    const device = String(platform?.device || '');
    return `${device} · ${provider}`;
}
function sortPlatformsForComparison(platforms) {
    return platforms
        .map((p) => {
        const provider = String(p.provider || '');
        const device = String(p.device || '');
...
function findDefaultComparePair(platforms, _preferredProvider = '') {
    const sorted = sortPlatformsForComparison(platforms);
    return [sorted[0] || null, sorted[1] || null];
}
function findDefaultComparePeer(provider, device) {
    ...
    return sortPlatformsForComparison(platformsIndexCache || [])
        .find((p) => p.provider !== currentProvider || p.device !== currentDevice) || null;
}
function normalizeCompareSelection(provider, device, fallback) {
    const exact = (platformsIndexCache || []).find((p) => String(p.provider || '') === provider && String(p.device || '') === device);
    ...
    return fields.map(([key, label]) => {
        const values = details.map((detail) => {
            ...
            if (Array.isArray(value))
                return value.length > 8 ? `${value.length} entries` : value.join(', ');
            if (value && typeof value === 'object')
...
function getProviderCompareTheme(provider) {
    const p = String(provider || '').toLowerCase();
    if (p.includes('ibm'))
        return 'ibm';
    if (p.includes('quantinuum') || p.includes('quantinum'))
        return 'quantinuum';
    if (p.includes('aws') || p.includes('amazon') || p.includes('braket'))
        return 'aws';
    if (p.includes('origin'))
        return 'origin';
    return 'default';
}
function getProviderCompareOptionStyle(provider) {
    const theme = getProviderCompareTheme(provider);
    if (theme === 'ibm')
        return 'background-color:#87ceeb;color:#0f172a;';
    if (theme === 'quantinuum')
        return 'background-color:#000;color:#fff;';
    if (theme === 'aws')
        return 'background-color:#ff9900;color:#111827;';
    if (theme === 'origin')
        return 'background-color:#fff;color:#111827;';
    return '';
}
function renderCompareMetricRow(label, aHtml, bHtml) {
    return renderCompareMetricRowHtml(escapeHtml(label), aHtml, bHtml);
}
function renderCompareMetricRowHtml(labelHtml, aHtml, bHtml) {
    return `<tr><th scope="row">${labelHtml}</th><td>${aHtml}</td><td>${bHtml}</td></tr>`;
}
function renderCompareMaybeBetterNumber(value, otherValue, digits) {
...
function renderCompareDeviceTitleHtml(provider, device, source) {
    return `<h4>${renderDeviceLabelHtml(provider, device, source)}</h4>`;
}
function renderCompareDeviceHeaderLink(provider, device) {
...
function renderCompareComponentRow(label, weightHtml, aHtml, bHtml, aHref = '', bHref = '') {
...
function sortCompareComponentNames(leftComponents, rightComponents) {
...
function renderComparePickerHtml(leftProvider, leftDevice, rightProvider, rightDevice) {
    const platforms = (platformsIndexCache || []).slice().sort((a, b) => renderPlatformOptionLabel(a).localeCompare(renderPlatformOptionLabel(b)));
    const optionHtml = (options, selectedProvider, selectedDevice) => options.map((p) => {
        const provider = String(p.provider || '');
        const device = String(p.device || '');
...
function formatCompareGraphNumber(value, digits = 3) {
...
function renderCompareGraphBars(leftLabel, rightLabel, leftValue, rightValue, scaleValue) {
    const finiteValues = [leftValue, rightValue].filter((value) => value !== null && Number.isFinite(value) && value > 0);
...
function renderCompareGraphValueCell(valueHtml, resultsHref = '', isBetter = false) {
...
function renderCompareGraphRow(label, leftValue, rightValue, leftLabel, rightLabel, differenceHtml, digits = 3, leftHref = '', rightHref = '', scaleValue) {
...
function bindCompareComponentResultCells(root) {
    root.querySelectorAll('.compare-component-result-cell[data-results-href]').forEach((cell) => {
        ...
        if (!href)
            return;
        const open = () => {
            const graphTable = cell.closest('.compare-graph-wrap');
            const componentSection = cell.closest('#compare-benchmark-components');
...
function bindCompareGraphNavigationAnchors(root) {
    root.querySelectorAll('.compare-graph-wrap a[href]').forEach((link) => {
...
function bindCompareHelpTooltips(root) {
    ...
    root.querySelectorAll('.compare-help[data-tip], .compare-score-help[data-tip]').forEach((el) => {
        if (el.dataset.tipBound === '1')
            return;
...
function renderPlatformComparePage(left, right) {
    const container = document.getElementById('platforms-container');
    if (!container)
        return;
    ...
    const metadataHtml = metadataRows.length ? metadataRows.map((row) => renderCompareMetricRow(row.label, escapeHtml(row.values[0]), escapeHtml(row.values[1]))).join('') : renderCompareMetricRow('Metadata', '–', '–');
    ...
    if (compareAnchor === 'components' || compareAnchor === 'graph') {
        requestAnimationFrame(() => {
            const target = compareAnchor === 'graph'
                ? container.querySelector('.compare-graph-wrap')
...
async function showPlatformDetailPage(provider, device) {
    const container = document.getElementById('platforms-container');
    if (!container)
        return;
    const config = await loadAppConfig();
...
function renderPlatformDetailPage(detail, suiteDefinition = null) {
  ...
  `;
    ...
    container.querySelectorAll('#platform-detail-table tbody tr[data-results-href]').forEach((row) => {
...
async function initPlatformsView(forceRender = false) {
    if (platformsLoaded && !forceRender)
        return;
    const container = document.getElementById('platforms-container');
    if (!container)
        return;
...
function getDeviceKey(provider, device) { return `${provider}::${device}`; }
function computeDeviceSeries(runs) {
    const weeks = 12;
    const now = Date.now();
    const weekMs = 7 * 24 * 3600 * 1000;
    const edges = Array.from({ length: weeks + 1 }, (_, i) => now - (weeks - i) * weekMs);
...
function renderSparkline(values, width = 100, height = 24, stroke = '#2563eb') {
...
function ensureGlobalTooltip() {
    ...
    document.addEventListener('keydown', (ev) => { if (!tip.hidden && ev.key === 'Escape')
        hide(); });
    return tip;
}
...
function ensureGlobalPopover() {
    ...
    if (!globalPopoverOutsideHandlerBound) {
        ...
        document.addEventListener('keydown', (ev) => { if (ev.key === 'Escape')
            globalPopoverCloseFn?.(); });
    }
    return pop;
}
function showGlobalPopover(anchorEl, html) {
...
function ensurePlatformsHeaderTooltipsBound(table) {
    ...
    const tipEls = table.querySelectorAll('.th-help[data-tip]');
...
function removeLegacyCompareColumn(table) {
    let removed = false;
    while (true) {
...
function ensurePlatformsProviderFilterBound(table) {
    const btn = table.querySelector('#platform-provider-filter-btn');
    if (btn && !btn.dataset.bound) {
        btn.addEventListener('click', (ev) => {
            ev.preventDefault();
            ev.stopPropagation();
            const existing = document.getElementById('global-popover');
            if (existing && !existing.hidden && existing.dataset.anchorId === btn.id) {
                closeGlobalPopover();
                return;
            }
            const providers = Array.from(new Set((Array.isArray(platformsIndexCache) ? platformsIndexCache : [])
                .map((p) => String(p?.provider || '').trim())
                .filter((s) => !!s))).sort((a, b) => a.toLowerCase().localeCompare(b.toLowerCase()));
            ...
            const optionsHtml = [
                ...
                ...providers.map((p) => (`<button type="button" class="popover-option${p === currentExact ? ' is-active' : ''}" data-provider="${escapeAttr(p)}">${escapeHtml(p)}</button>`)),
            ].join('');
      ...
      `.trim());
            ...
            const optionBtns = pop?.querySelectorAll('button.popover-option[data-provider]');
...
function renderPlatformsTable() {
    ...
    platforms.forEach((p) => {
        ...
        if (v !== undefined && Number.isFinite(v))
            displayScores.set(key, Number(v));
    });
    let wrap = document.getElementById('platforms-table-wrap');
    if (wrap) {
        wrap.remove();
        wrap = null;
    }
    ...
    const providerTh = table.querySelector('thead th[data-col="provider"]');
    if (providerTh)
        providerTh.innerHTML = renderPlatformsProviderHeaderHtml();
    ensurePlatformsProviderFilterBound(table);
    ...
    if (!tbody)
        return;
    const maxScore = filtered.reduce((max, p) => {
        const key = getDeviceKey(String(p.provider || ''), String(p.device || ''));
    ...
    if (table && table.dataset) {
        ...
        if (!dataTable.dataset.scoreClickBound) {
            table.addEventListener('click', (event) => {
                const target = event.target;
                const cell = target && target.closest ? target.closest('td.metriq-score') : null;
                if (cell) {
    ...
    const headCells = table.querySelectorAll('thead th[data-col]');
...
function adaptMetriqEtlRow(row) {
    ...
    if (rawScore && typeof rawScore === 'object' && 'value' in rawScore) {
        score = parseFinite(rawScore.value);
    }
    else {
...
function normalizeRun(run) {
    ...
    const metrics = (clone && typeof clone.metrics === 'object' && clone.metrics != null)
        ? { ...clone.metrics }
        : {};
    const errors = (clone && typeof clone.errors === 'object' && clone.errors != null)
        ? { ...clone.errors }
        : {};
...
function resolveBenchmarkUrl(query) {
    if (!benchmarkPages.length)
        return null;
    if (!query)
        return benchmarkPages[0].url;
...
if (detailModal) {
    detailModal.addEventListener('click', (event) => {
        ...
        if (t === detailModal || (t && typeof t.hasAttribute === 'function' && t.hasAttribute('data-detail-close'))) {
            closeDetail();
        }
    });
}
...
function setupFilterSearchInputs() {
    ...
    const benchSearch = document.getElementById('filter-search-benchmark');
    const provSearch = document.getElementById('filter-search-provider');
    if (benchSearch)
        benchSearch.addEventListener('input', handler);
    if (provSearch)
        provSearch.addEventListener('input', handler);
}
...
function getFilteredData() {
    ...
    if (selProv.length === 0 || selBench.length === 0)
        return [];
...
function renderRawResults(run) {
    try {
        ...
        if (!keys.length)
            return '<div class="meta">No raw results available.</div>';
...
async function renderChart(values, token, metric) {
    ...
    if (skeletonGraph)
        skeletonGraph.style.display = "block";
    ...
    const metricVals = values.map((d) => d.metricValue).filter((v) => v != null && isFinite(v));
    const yMin = Math.min(...metricVals);
    const yMax = Math.max(...metricVals);
    const yPad = (yMax - yMin) * 0.1 || 1;
    const yScale = { type: scaleType, nice: true, domain: [Math.max(0, yMin - yPad), yMax + yPad] };
    const timestamps = values.map((d) => new Date(d.timestamp).getTime()).filter((t) => isFinite(t));
    const tMin = Math.min(...timestamps);
    const tMax = Math.max(...timestamps);
    const tPad = (tMax - tMin) * 0.05 || 86400000;
    const xScale = { domain: [new Date(tMin - tPad).toISOString(), new Date(tMax + tPad).toISOString()] };
    ...
    const colorRange = providers.map(p => providerColorMap.get(p));
    const shapeDomain = benchmarks;
    const shapeRange = benchmarks.map(b => benchmarkShapeMap.get(b));
    ...
    const annotationMark = { type: 'text', align: 'left', dx: 10, dy: -6, fontSize: 11, fontWeight: 600, font: 'Inter, system-ui, sans-serif', clip: true };
    const annotationEncoding = {
        y: { field: 'metricValue', type: 'quantitative', scale: yScale },
        text: { field: 'device', type: 'nominal' },
        color: { field: 'provider', type: 'nominal', legend: null, scale: { domain: colorDomain, range: colorRange } }
    };
    const topPerformerLayer = [
...
function ensureTableUI() {
    ...
    if (!container)
        return null;
    if (container.getAttribute('data-smart') === '1')
        return container;
...
function getMetricSortValue(run, metricId) {
    const v = getMetricValue(run, metricId);
    return Number.isFinite(v) ? v : Number.NEGATIVE_INFINITY;
}
function populateSmartFilters(values) {
    const provSel = document.getElementById('smart-provider');
    const devSel = document.getElementById('smart-device');
    const benchSel = document.getElementById('smart-benchmark');
...
function applyTableFilters(values) {
    const q = (tableState.filterText || '').toLowerCase();
    return values.filter(v => {
        if (tableState.filterProvider !== 'all' && String(v.provider || '') !== tableState.filterProvider)
            return false;
        if (tableState.filterDevice !== 'all' && String(v.device || '') !== tableState.filterDevice)
            return false;
...
function sortTableRows(values) {
    const { sortKey, sortDir } = tableState;
    const mul = sortDir === 'asc' ? 1 : -1;
    const cmp = (a, b) => {
        let av, bv;
        if (sortKey === 'timestamp') {
            av = Number(new Date(a.timestamp));
            bv = Number(new Date(b.timestamp));
        }
        else if (sortKey === 'num_qubits') {
            av = Number(a.num_qubits ?? Number.NEGATIVE_INFINITY);
            bv = Number(b.num_qubits ?? Number.NEGATIVE_INFINITY);
        }
        else if (sortKey === 'provider') {
            av = String(a.provider || '');
            bv = String(b.provider || '');
        }
            ...
            av = String(a.benchmark || '');
            bv = String(b.benchmark || '');
        }
...
function renderStaticTable(values) {
    ...
    if (!container || !wrap)
        return;
    if (skeletonTable)
        skeletonTable.style.display = 'block';
    const metric = getActiveMetric();
    ...
    table.className = 'smart-table';
    const sortIcon = (key) => (tableState.sortKey === key
    ...
    if (!metricDefs.length) {
        const ids = Array.from(collectMetricIdsWithValues(working));
        metricDefs = ids.map(id => ({ id, label: id, unit: '', scale: 'linear', format: null }));
    }
    const metricHeaders = metricDefs.map((def) => `<th data-sort="metric:${escapeAttr(def.id)}" class="sortable num">${escapeHtml(buildMetricLabel(def))}${sortIcon(`metric:${def.id}`)}</th>`).join('');
    table.innerHTML = `
	    <thead>
	      <tr>
    ...
    working.forEach(run => {
      ...
      <td class="num">${escapeHtml(formatDateOnly(run.timestamp))}</td>`;
        ...
        const scoreLink = tr.querySelector('a.metric-link[data-role="score"]');
        if (scoreLink) {
        ...
        const deviceLink = tr.querySelector('a.metric-link[data-role="device"]');
        if (deviceLink) {
        ...
        const benchLink = tr.querySelector('a.metric-link[data-role="benchmark"]');
        if (benchLink) {
    ...
    const qInput = document.getElementById('smart-q');
    const provSel = document.getElementById('smart-provider');
    const devSel = document.getElementById('smart-device');
    const benchSel = document.getElementById('smart-benchmark');
    const resetBtn = document.getElementById('smart-reset');
    let qTimer;
    if (qInput) {
...
async function drawChart() {
    ...
    const chartValues = dedupedRuns
        ...
        if (metricValue === null)
            return null;
        if (metric.scale === 'log' && metricValue <= 0)
            return null;
...
async function initBenchmarksView() {
    ...
    try {
        ...
        const platforms = Array.isArray(platformData?.platforms) ? platformData.platforms : [];
        setPlatformsIndexCache(platforms);
...
async function injectFooter() {
    ...
    try {
        ...
        if (!resp.ok)
            throw new Error(`HTTP ${resp.status} loading footer.html`);
