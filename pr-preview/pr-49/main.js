...
const heroNewsLead = document.getElementById('hero-news-lead');
...
function activateView(which, skipHashUpdate = false) {
    ...
    if (viewNews)
        viewNews.hidden = !isNews;
    if (insightsSection)
        insightsSection.hidden = isNews;
...
function focusNewsFromHash() {
    if (!viewNews || viewNews.hidden)
        return;
    const targetId = String(parseHash().update || '').trim();
    const cards = Array.from(viewNews.querySelectorAll('[data-update-id]'));
...
function initUpdates() {
    if (!updatesPromise)
        updatesPromise = loadAppConfig().then(renderUpdates);
    return updatesPromise;
}
function renderUpdateCard(u, archive = false) {
...
async function renderUpdates(config) {
    ...
    if (!section || !track || !newsList || !newsStatus)
        return;
    ...
    try {
        ...
        if (!Array.isArray(json))
            throw new Error('Invalid updates list');
        items = json;
    }
    catch (err) {
    ...
    bindCaptionLinks(Array.from(document.querySelectorAll('.update-card [data-news-link]')), navigateToHash);
    syncRecordModeToggle();
...
bindCaptionLinks(Array.from(document.querySelectorAll('[data-news-link]')), navigateToHash);
...
function renderCompareComponentRow(label, weightHtml, aHtml, bHtml, differenceHtml, aHref = '', bHref = '') {
    return renderCompareComponentRowHtml(escapeHtml(label), weightHtml, aHtml, bHtml, differenceHtml, aHref, bHref);
}
...
function renderComparePickerHtml(leftProvider, leftDevice, rightProvider, rightDevice) {
    ...
    const comparisonPeers = platforms.filter((platform) => (String(platform.provider || '') !== leftProvider || String(platform.device || '') !== leftDevice));
    return `
...
function bindCompareComponentResultCells(root) {
    root.querySelectorAll('.compare-component-result-cell[data-results-href]').forEach((cell) => {
        ...
        const open = () => {
            ...
            componentSection?.scrollIntoView({ block: 'start', behavior: 'auto' });
            ...
            if (current.view === 'platforms' && current.compare_provider_a && current.compare_device_a && current.compare_provider_b && current.compare_device_b) {
                current.compare_anchor = 'components';
...
function renderPlatformComparePage(left, right) {
    ...
    const componentRows = componentNames.length ? componentNames.map((name) => {
  ...
  `;
    ...
    if (compareAnchor === 'components' || compareAnchor === 'graph') {
        requestAnimationFrame(() => {
            const target = document.getElementById('compare-benchmark-components');
