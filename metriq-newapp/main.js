...
function renderMultiList(listId, options, selected, kind) {
...
function renderMultiLists() {
    ...
    if (pClear)
        pClear.onclick = () => { filterState.provider = []; renderMultiLists(); drawChart(); };
    if (pAll)
        pAll.onclick = () => { filterState.provider = providers.slice(); renderMultiLists(); drawChart(); };
    if (bClear)
        bClear.onclick = () => { filterState.benchmark = []; renderMultiLists(); drawChart(); };
    if (bAll)
        bAll.onclick = () => { filterState.benchmark = benchmarks.slice(); renderMultiLists(); drawChart(); };
}
...
function renderPlatformDetailPage(detail) {
    ...
    if (metriqScore && typeof metriqScore === 'object') {
        const val = Number(metriqScore.value);
...
async function initPlatformsView(forceRender = false) {
    ...
    `;
        ...
        container.addEventListener('click', (ev) => {
            ...
            const btn = (target && target.closest('button[data-provider][data-device]'));
            if (btn) {
...
function renderPlatformsTable() {
    ...
    const providers = Array.from(new Set(platforms.map((p) => String(p.provider || '')))).sort();
    let controls = document.getElementById('platform-controls');
    let wrap = document.getElementById('platforms-table-wrap');
    let table = wrap ? wrap.querySelector('table') : null;
    ...
    `;
      ...
      <tbody></tbody>`;
        ...
        const qInput = controls.querySelector('#platform-q');
        const provSel = controls.querySelector('#platform-provider');
        const resetBtn = controls.querySelector('#platform-reset');
        let qTimer;
        if (qInput) {
    ...
    const provSel = controls.querySelector('#platform-provider');
    if (qInput)
        qInput.value = platformFilterText;
    if (provSel) {
        ...
        if (platformFilterProvider === 'all')
            provSel.value = 'all';
        else if (providers.includes(platformFilterProvider))
            provSel.value = platformFilterProvider;
    }
    const term = (platformFilterText || '').toLowerCase();
    const filtered = platforms.filter((p) => {
        if (platformFilterProvider !== 'all' && String(p.provider || '') !== platformFilterProvider)
            return false;
        if (term) {
            const prov = String(p.provider || '').toLowerCase();
            const dev = String(p.device || '').toLowerCase();
            if (!prov.includes(term) && !dev.includes(term))
                return false;
        }
        return true;
    });
...
function renderStaticTable(values) {
    ...
    const table = document.createElement('table');
    table.className = 'smart-table';
    const sortIcon = (key) => tableState.sortKey === key ? (tableState.sortDir === 'asc' ? ' ▲' : ' ▼') : '';
